...
| **Actions/Docker/Terraform** (`infra`, `.github`, `mysql`, `redis`) | none — rely on Dependabot (Phase 2) for action SHAs, base images, providers | bump pinned action SHAs / `FROM` base tags / TF provider versions / Ansible collections as the alert dictates | infra: `ansible-lint` + `ansible-playbook site.yml --syntax-check` (see project CLAUDE.md for the env it needs); Docker repos: `make build` |
